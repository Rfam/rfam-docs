...
.. hint::
    Consider also reviewing the ``outlist`` file which is similar to ``outlist`` but contains slightly different information, such as sequence descriptions as well as the details about whether the hits were truncated or appear on the reverse strand.
