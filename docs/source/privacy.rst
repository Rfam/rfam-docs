...
Google Analytics
----------------
We use `Google Analytics <https://developers.google.com/analytics>`_ (GA) to track the usage of this website. GA uses a single-pixel "web bug" image, which is served from every page, a javascript script that collects information about each request, and cookies that maintain information about your usage of the site between visits. You can read more about how GA works on the `Google Analytics <https://www.google.com/analytics/>`_ website, which includes a `detailed <https://developers.google.com/analytics/devguides/reporting/data/v1>`_ description of how traffic is tracked and analysed.
...
Although our servers do collect information about your `IP address <https://en.wikipedia.org/wiki/IP_address>`_ during the normal process of serving the Rfam website, we do not use this information explicitly. The Rfam group uses server logs **only** to help with development and debugging of the site.
...
Cookies
-------
We use the following `cookie <https://en.wikipedia.org/wiki/HTTP_cookie>`_ to maintain some information about you between your visits to the site. The information that is stored cannot be used to identify you personally and cannot be used to track your usage of the site.
...
In addition to this Rfam-specific cookie, `GA uses a series of cookies <https://developers.google.com/analytics>`_. You can read more about these in the GA documentation , or in EMBL-EBI's `cookie policy <https://www.ebi.ac.uk/about/terms-of-use/#cookies>`_.
